...
class Container extends React.Component {
    ...
    render() {
        ...
        if (currentItem !== null) {
            const icon = currentItem.typeToast === 'error' ? 'fa-circle-exclamation' : (currentItem.typeToast === 'info' ? 'fa-circle-info' : 'fa-triangle-exclamation');
            ...
            return (<div className={`wrapper p-5 rounded ${classLoad} `} onClick={(e) => this.hideToast(e)}>
                <div className={`toast ${currentItem.typeToast} ${classLoad} `}>
                    <div className="content">
                        <div className={`icon ${typeIconClass}`}><i className={`fa-solid ${icon}`}></i></div>
                        <div className="details">
                            <span>{currentItem.title}</span>
                            <p>{currentItem.text}</p>
                        </div>
                    </div>
                    <div className={`icon`} onClick={(e) => this.hideToast(e)}><i className="fa-solid fa-xmark"></i></div>
                </div>
            </div>);
        }
        return null;
